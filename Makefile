...
oci-images:
	...
	docker buildx build --build-arg TOKEN_TYPE=mskey -t $(IMAGE_NAME_BASE):mskey -t $(IMAGE_NAME_BASE):mskey-$(CI_BUILD_NUMBER) .
	docker buildx build --build-arg TOKEN_TYPE=jacarta -t $(IMAGE_NAME_BASE):jacarta -t $(IMAGE_NAME_BASE):jacarta-$(CI_BUILD_NUMBER) .
